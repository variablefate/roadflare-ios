...
extension NostrFilter {
    /// Filter for ride history backup (Kind 30174, encrypted to self).
    public static func rideHistoryBackup(myPubkey: String) -> NostrFilter {
        NostrFilter()
            .kinds([.rideHistoryBackup])
            .authors([myPubkey])
            .dTags(["rideshare-history"])
            .limit(1)
    }
