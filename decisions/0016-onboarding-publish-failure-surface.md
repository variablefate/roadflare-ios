...
4. **Banner placement.** `OnboardingPublishFailureBanner` is rendered as the first child of a `VStack` in `RootView`, above the auth-state switch (`@ViewBuilder` `authStateContent`). SwiftUI's default safe-area handling places the banner *content* below the status bar, but the *strip above* the banner is not covered by any single subview's background — each auth-state view's own `Color.rfSurface.ignoresSafeArea()` only extends adjacent to its own frame, which when banner is showing no longer reaches the status-bar zone. The VStack itself paints `Color.rfSurface.ignoresSafeArea()` as its background so the status-bar / Dynamic Island zone stays on-brand in both banner-visible and banner-hidden states. The banner copy is domain-specific ("Your profile hasn't been backed up yet" / "Your profile and settings haven't been backed up yet"). Retry button is a discrete focusable accessibility element.
...
- **`RootView` is now a `VStack`-rooted view** (banner + auth-state content) rather than a bare auth-state switch. The auth-state subviews retain their own `Color.rfSurface.ignoresSafeArea()` backgrounds, and the VStack itself paints `Color.rfSurface.ignoresSafeArea()` as its background so the status-bar / Dynamic Island zone stays on-brand even when the banner is showing (each subview's own `ignoresSafeArea` background can no longer reach the strip above the banner).
